...
class ProductModel extends BaseModel
{
    ...
    function findAllGroupBy($filters=array(),$paginator=array()){
        ...
        $query = 'SELECT *, SUM(stock) as stock, "0.0" as price, "" as model FROM '.$this->tableName .( empty($filters) ?  '' : ' WHERE '.$conditions ).' group by item ORDER BY item DESC LIMIT '.$paginator['limit'].' OFFSET '.$paginator['offset'];
